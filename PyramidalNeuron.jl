...
function simulatePyC(t, v_d, w_d, v_s, w_s, I_inj_d, I_inj_s, I_dbg, I_sbg, t_, t_soma, st_SSTE, st_PVE, st_EE)
    I_dbg += dI_dbg_dt(I_dbg) .* dt
    I_sbg += dI_sbg_dt(I_sbg) .* dt

    v_d += dv_d_dt(v_d, I_inj_d, I_dbg, w_d, st_SSTE, t_soma, t) .* dt
    w_d += dw_d_dt(w_d, v_d) .* dt

    v_s += dv_s_dt(v_s, v_d, I_inj_s, I_sbg, w_s, st_PVE, st_EE) .* dt
