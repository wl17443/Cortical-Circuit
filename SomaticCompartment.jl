...
dv_s_dt(v_s, v_d, I_inj, I_sbg, w_s, st_PVEs, st_EE) = -(v_s .- EL) ./ t_s + (g_s .* f(v_d) + w_s + I_inj + I_sbg + I_s_pv(st_PVEs) + I_s_e(st_EE)) ./ C_s
...
I_s_pv(st_PVEs) = -sum(abs.(W_PVEs) * st_PVEs, dims=2)
I_s_e(st_EE) = sum(abs.(W_EE) * st_EE, dims=2)
