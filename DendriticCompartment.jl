...
function K(t)
    for i = 1:length(t)
        ...
        elseif t[i] > 1*ms && t[i] < 3*ms
            t[i] = 0*mV ## TODO - Change to actual plateau value
        else t[i] = -70*mV end 
    end 
    return t
end 
...
dv_d_dt(v_d, I_inj, I_dbg, w_d, t_, W_SSTEd, st_SSTEd, t) = -(v_d .- EL) ./ t_d + (g_d .* f(v_d) + c_d .* K(t*dt .- t_) + w_d + I_inj + I_dbg + I_d_sst(W_SSTEd, st_SSTEd)) ./ C_d
