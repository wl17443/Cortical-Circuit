...
## Simulation
for t = 2:steps
    v_d[:, t], w_d[:, t], v_s[:, t], w_s[:, t], I_dbg[:, t], I_sbg[:, t], t_pyc[:, t], t_soma[:] = simulatePyC(t, v_d[:, t-1], w_d[:, t-1], v_s[:, t-1], w_s[:, t-1], I_inj_d[:, t-1], I_inj_s[:, t-1], I_dbg[:, t-1], I_sbg[:, t-1], t_pyc[:, t-1], t_soma, st_SSTEd, st_PVEs)

    ## Simulate for each type of interneuron 
    v_sst[:, t], I_sstbg[:, t], t_sst[:, t], tspike_sst[:] = simulateI("SST", t, v_sst[:, t-1], I_sstbg[:, t-1], tspike_sst, st_EsSST, st_PVSST) 
    v_pv[:, t], I_pvbg[:, t], t_pv[:, t], tspike_pv[:] = simulateI("PV", t, v_pv[:, t-1], I_pvbg[:, t-1], tspike_pv, st_EsPV, st_SSTPV) 

    ## Update synaptic trace 
    for i=1:nr_pyc, j=1:nr_sst
        st_EsSST[i, j] += (- st_EsSST[i, j] ./ tau_syn) * dt + t_pyc[i, t]
    end 
    for i=1:nr_pyc, j=1:nr_pv
        st_EsPV[i, j] += (- st_EsPV[i, j] ./ tau_syn) * dt + t_pyc[i, t]
    end 
    for i=1:nr_sst, j=1:nr_pyc
        st_SSTEd[i, j] += (- st_SSTEd[i, j] ./ tau_syn) * dt + t_sst[i, t]
    end 
    for i=1:nr_pv, j=1:nr_pyc
        st_PVEs[i, j] += (- st_PVEs[i, j] ./ tau_syn) * dt + t_pv[i, t]
    end 
...
p12 = plot(step_list, v_s[1, :], label="PyC1")
p13 = plot(step_list, v_s[2, :], label="PyC2")
p14 = plot(step_list, v_s[3, :], label="PyC3")
p15 = plot(step_list, v_s[4, :], label="PyC4")
p16 = plot(step_list, v_s[5, :], label="PyC5")
